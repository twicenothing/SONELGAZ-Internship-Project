...
def load_old_data_gaz_TB(file=TB):
    ...
    keywords=['Nombre d\'abonnés','Accroissement','II-  Réabonnement']
    table_titles = detect_tables(df, keywords)
    start = table_titles[2]-2
    end = table_titles[2]+10
    reabonne = df.iloc[start:end]
    # reabonne = reabonne.dropna(axis='columns',how='all')
    reabonne = reabonne.iloc[:,:53]
    # reabonne = reabonne.replace(0,np.nan)
    ...
    apport_nouveau.index.name = None
    # apport_nouveau = apport_nouveau.replace(np.nan, 0)
    keywords=['Nombre d\'abonnés','Accroissement','III-  Ventes gaz']
    table_titles = detect_tables(df, keywords)
    start = table_titles[2]-3
    end = table_titles[2]+9
    ventes = df.iloc[start:end]
    ...
    keywords=['Nombre d\'abonnés','Accroissement','IV-  Achats gaz & pertes d\'énergie']
    table_titles = detect_tables(df, keywords)
    start = table_titles[2]-4
    end = table_titles[2]+8
    achats = df.iloc[start:end]
    # achats = achats.dropna(axis='columns', how='all')
    achats = achats.iloc[:, :53]
    # achats = achats.replace(0, np.nan)
    ...
    keywords=['Nombre d\'abonnés','Accroissement','V-  Chiffre d\'affaires gaz']
    table_titles = detect_tables(df, keywords)
    start = table_titles[2]-6
    end = table_titles[2]+6
    chiffre_aff = df.iloc[start:end]
    # chiffre_aff = chiffre_aff.dropna(axis='columns', how='all')
    chiffre_aff = chiffre_aff.iloc[:, :53]
    # chiffre_aff = chiffre_aff.replace(0, np.nan)
    ...
    chiffre_aff.index.name = None
    # chiffre_aff = chiffre_aff.replace(np.nan, 0)
    keywords=['Nombre d\'abonnés','Accroissement','VI- Prix de Ventes Moyen gaz']
    table_titles = detect_tables(df, keywords)
    start = table_titles[2]-7
    end = table_titles[2]+5
    prix = df.iloc[start:end]
    # prix = prix.dropna(axis='columns', how='all')
    prix = prix.iloc[:, :53]
    # prix = prix.replace(0, np.nan)
...
def TB_gaz():
   # Load data for tables
    n, a, ap, r, rea, apnv, ventesTB, achatsTB, chiffaf, pr = load_old_data_gaz_TB()
    w = nombre_abo_tb_gaz()
    nombre_abbo_gaz = accroissement_tb_gaz()
    accroissement_elec = resiliation_tb_gaz()
    tx_encaissement = apport_tb_gaz()
    apport_elec = reabonne_tb_gaz()
    apport_gaz = apport_nv_tb_gaz()
    evolution = ventes_tb_gaz()
    evolution2 = achats_tb_gaz()
    chiffre_aff = chiffre_aff_tb_gaz()
    prix = prix_tb_gaz()
